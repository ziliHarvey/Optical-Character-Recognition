...
for k,v in params.items():
    #k is key of params, v is values of params
    if '_' in k: 
        continue #k: Wlayer1, Woutput, blayer1, boutput
    ...
    flatv = v.ravel()
    flatg = g.ravel()
    #calculate loss(x+eps)
    for i in range(flatv.shape[0]):
        flatv[i] += eps
        h1_add = forward(x,params,'layer1')
        probs_add = forward(h1_add,params,'output',softmax)
        loss_add, _ = compute_loss_and_acc(y, probs_add)
        #calculate loss(x-eps)
        flatv[i] -= 2*eps
        h1_sub = forward(x,params,'layer1')
        probs_sub = forward(h1_sub,params,'output',softmax)
        loss_sub, _ = compute_loss_and_acc(y, probs_sub)
        #compute der with central diffs
        flatg[i] = (loss_add - loss_sub)/(2*eps)
        flatv[i] += eps
    params['grad_' + k] = g
